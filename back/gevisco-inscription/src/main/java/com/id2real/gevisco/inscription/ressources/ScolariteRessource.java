...
import com.id2real.gevisco.inscription.services.ScolariteServiceInt;
import com.id2real.gevisco.inscription.utils.InscriptionConstants;
import com.id2real.socle.utils.StringUtil;
import com.querydsl.core.BooleanBuilder;
@RestController
@RequestMapping(InscriptionConstants.API_INSCRIPTION_SCOLARITE_URL)
public class ScolariteRessource {

	@Autowired
	private ScolariteServiceInt scolariteService;
	
	 @GetMapping("/find/{id}")
	    @ResponseStatus(code = HttpStatus.OK)
	    public ScolariteDTO find(@PathVariable("id") String id) {
	        return this.scolariteService.getDTO(id);
	    }

	    @GetMapping("/count")
	    @ResponseStatus(code = HttpStatus.OK)
	    public long count() {
	        return scolariteService.count();
	    }
	   ...
	   // @PreAuthorize("hasAnyAuthority('" + CoreConstants.SUPERADMIN_ROLE + "','" + TiersConstants.FC_TIERS_CREATE + "')")
	    public ScolariteDTO save(@RequestBody ScolariteDTO dto) {
	        return this.scolariteService.saveDTO(dto);
	    }
	    ...
	    public ScolariteDTO update(@RequestBody ScolariteDTO dto) {
//	        return AnneeScolaireDTO.updateDTO(dto.getId(), dto);
	        return scolariteService.saveDTO(dto);
	    }
	    ...
	    @RequestMapping( method = RequestMethod.GET)
	       public List<ScolariteDTO> findAll() {
	    	/*
	        ...
	        return scolariteService.findAllDTO();
	    }
