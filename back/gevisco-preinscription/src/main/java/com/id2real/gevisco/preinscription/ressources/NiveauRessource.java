...
import com.id2real.gevisco.preinscription.entities.QNiveau;
...
@RestController
@RequestMapping(PreinscriptionConstants.API_PREINSCRIPTION_NIVEAU_URL)
public class NiveauRessource {
	...
	private NiveauServiceInt niveauService;
	 ...
	 @GetMapping("/find/{id}")
	    ...
	    public NiveauDTO find(@PathVariable("id") String id) {
	        return this.niveauService.getDTO(id);
	    }
	   ...
	   // @PreAuthorize("hasAnyAuthority('" + CoreConstants.SUPERADMIN_ROLE + "','" + TiersConstants.FC_TIERS_CREATE + "')")
	    public NiveauDTO save(@RequestBody NiveauDTO dto) {
	        return this.niveauService.saveDTO(dto);
	    }
	    ...
	    public NiveauDTO update(@RequestBody NiveauDTO dto) {
//	        return AnneeScolaireDTO.updateDTO(dto.getId(), dto);
	        return niveauService.saveDTO(dto);
	    }
	    ...
	    public List<NiveauDTO> findAll() {
	        ...
	        if (motcle.isPresent()) {
	            for (String mot : StringUtil.splitTextToWords(motcle.get())) {
	                builder.or(QNiveau.niveau.libelle.toUpperCase().containsIgnoreCase(mot.toUpperCase()));
	            }
	        }*/ 
	        return niveauService.findAllDTO();
	    }
